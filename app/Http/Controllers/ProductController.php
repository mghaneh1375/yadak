...
class ProductController extends Controller {
    ...
    public function productsInCategory($id) {
        ...
        foreach ($products as $product) {
            ...
            if($product->secondary_price != null) {
    ...
    public function saveExcelProduct()
    {
        ...
        $err = $this->groupRegistration($categoryId);
        return redirect(url('addProduct'));
    }
    
    private function groupRegistration($categoryId)
    {
    ...
    private function addProductExcel($products, $items, $categoryId)
    {
        ...
        for ($i = 0; $i < count($products); $i++) {
            $brand =  Brand::whereName($products[$i][$brandNum])->whereCategoryId($categoryId)->first();
            if($brand == null) {
                $brand = new Brand();
                $brand->name = $products[$i][$brandNum];
